...
SITE_CONCEPT = (
    ...
    "（それは AI に任せてよい）。目標は、"
    "<strong>書かれたコードを読めるようになること——すなわち項の型を推測できるようになること。"
    "その推測は機械的に行えるので、コンピュータにも行える</strong>。"
    "<strong>そして、この型を推測する仕組みが定理の証明の検証にそのまま使える、と納得すること</strong>。"
    "その上で、<strong>形式化を自分の研究に役立てる可能性を考えること</strong>である。"
)
SITE_NOTE = (
