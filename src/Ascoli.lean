...
`Top.lean` まで読み終えた人のための、もう1つの演習問題集
（`Extra.lean` とは独立に読める）。題材は関数空間のコンパクト性の定理
**Ascoli の定理**である。
...
* 有限性は `List` で扱う（`Top.lean` の `Set.Finite` との橋渡しは与える）
...
`Extra.lean` と同じく、`sorry` を自分の証明で置き換える。解答は `AscoliSol.lean` にある。
