...
`CH1.lean` では、含意と全称を関数と適用として読んだ。
`Intro1b.lean` では、構成子で項を作り、場合分けや射影で使う道具を学んだ。
この章で二つを合わせ、かつ・または・矛盾・存在の証明を読む。

偶数の和、全射の合成へ進み、最後に型と命題の対応、等式の仕組み、
検査の詳細を整理する。使った道具を一つずつ型の照合に戻して確かめよう。
-/

/-!
## 1. 直積型と「かつ」 {#sec-CH.products}
...
[`Intro1b.lean` 2節](#sec-Intro1.structures)でフィールドを番号で指定したのと同じ記法である。
-/
...
一点側の対応物もある。`Unit`（構成子1つ・引数なしの型、[`Intro1b.lean` 1節](#sec-Intro1.inductive-types)）に
対応する命題が `True` である:
...
そして、構成子が**複数**ある命題の代表が、[2節](#sec-CH.sums)で見た「または」`Or` だった
...
/-!
## 4. 存在を示す・使う — 偶数と全射 {#sec-CH2.existence}
...
理由は[10節](#sec-CH.prop-elimination)で見る。

`CH1.lean` で学んだ全称と、いま見た存在を組み合わせよう。
...
**項**。これを Lean に持ち込む。CH1 で学んだ全称と、直前の小例で見た存在を使うと、
...
CH1 で見た `theorem` の形に当てはめ、コロンの右に命題、`:=` の右に
...
証明も項なので、[`Intro1b.lean` 2節](#sec-Intro1.structures)の `p.swap` と同じく、
...
* `fun n =>` が「n を任意に取る」——全称の証明は、**どの n を渡されても、
  その n での証明を返す関数**になる（[`Intro1a.lean` 3節](#sec-Intro1.functions)の
  `fun` そのもの）。
* `⟨n, …⟩` が「候補と、性質を満たす証明」——存在命題の構成子も
  [`Intro1b.lean` 2節](#sec-Intro1.structures)の組と同じ `⟨ ⟩` 記法で書ける。
  この候補のことを以後**証人**と呼ぶ。
...
`match` の枝に入ったあとの型だけを見れば、`CH1.lean` の modus ponens と同じ関数適用である。
-/
...
**項**。「取り出して名前を付ける」は、[`Intro1b.lean` 1節](#sec-Intro1.inductive-types)の
...
`Sort` 自体の読み方は[8節](#sec-CH.nat-proofs)で整理する）:
...
∀ と → を [`Intro1a.lean` 3節](#sec-Intro1.functions)の binder 形式で書いた
...
**項**。帰納法は、[`Intro1b.lean` 1節](#sec-Intro1.inductive-types)の**再帰**
そのものとして書ける。「k での場合の証明」は、自分自身の再帰呼び出しで
手に入る（`toN` の再帰と同じ形である。また、計算で一致する等式の証明は
`rfl` と書く——[`CH1.lean` の1節](#sec-CH.propositions)で見た）:
-/
...
[`CH1.lean` の3節](#sec-CH1.forall-examples)の単射の合成と並べると対になっている: 単射は `fun` と適用だけの世界、全射は存在の
...
/-!
## 5. 計算で一致する等式と、帰納法 {#sec-CH2.computation}

### 補足（初読は飛ばしてよい）: 対称に見えて `rfl` が通らない `0 * n`

`CH1.lean` の `all_mul_zero` と同じ調子で `∀ n, IsZero (0 * n)` も書きたくなるが、こちらは通らない:
...
例えば関数 `fun x => f x` と `f` の同一視や、[`CH1.lean` の1節](#sec-CH.propositions)の証明無関連性も含まれる。
...
/-!
## 6. 整理 — 依存和と「存在する」 {#sec-CH.dependent-sums}

[4節](#sec-CH2.existence)では `∃` の証明を `⟨証人, 根拠⟩` で作り、`match` で使った。ここでは、
それを型の世界の依存和と並べて整理する。
...
データとして第一成分を射影する操作との違いは、[10節](#sec-CH.prop-elimination)で詳しく見る。
-/
...
`Intro1a.lean` では「`n` 未満の番号の型」`Fin n` を使った。
いま、その中身を開こう。値と証明を組にした次の structure である:
...
（[`Intro1a.lean` 3節](#sec-Intro1.functions)）。
...
（[`Intro1a.lean` 3節](#sec-Intro1.functions)の `2 : Int` と同じ仕組み）。
...
（その仕組みは [`Intro2.lean` 1節](#sec-Intro2.classes)の補足で見る）。
...
| 依存和 | `(a : α) × P a` | `∃ a, Q a` | `⟨a, b⟩` | `.1` / `.2`（`∃` は `match` のみ——[10節](#sec-CH.prop-elimination)） |
...
/-!
## 8. 例: 自然数の命題を構成子で証明する {#sec-CH.nat-proofs}

「命題を証明する＝項を作る」——つまり、**命題に対応する型の項を作れれば、
証明できたと思ってよい**——を、自然数のいちばん基本的な命題で確かめておく。
鍵は、`=` や `≤` といった命題を作るもの自体が、`Intro1b.lean` の `MyNat` などと
...
証明のない命題は、項のない型 `Empty`（[3節](#sec-CH.empty-types)）と同じ立場にある。

[`CH1.lean` の1節](#sec-CH.propositions)で `1 + 1 = 2` が `rfl` で証明できたのは、型検査が両辺を
...
`Eq` とその構成子の定義は[8節](#sec-CH.nat-proofs)で見る。
...
[`Intro1a.lean` 2節](#sec-Intro1.definitions)以来の検査が、そのまま**証明の検査**になっている。
[4節](#sec-CH2.existence)で行った証明項の検査も、すべてこの見方の繰り返しだった。
-/
...
/-!
これが[7節](#sec-CH.introduction-elimination)の主張の実演である。`0 ≤ 2` を証明するとは、帰納型 `Nat.le` の
項を構成子から組み立てることに他ならず、出来上がった証明は
データと同じ資格でそこにある。`∧` `∨` `∃` `False` も同様に帰納型として
定義された命題であり（[1](#sec-CH.products)〜[3節](#sec-CH.empty-types)、[6節](#sec-CH.dependent-sums)）、示すときに使った `⟨_, _⟩` や `.inl` は、
それらの構成子だったのである。
...
[`Intro1b.lean` 1節](#sec-Intro1.inductive-types)で、帰納型の宣言は2つのことを保証すると述べた:
...
ここで [`Intro1b.lean` 1節](#sec-Intro1.inductive-types)の原則——帰納型の項は、構成子で作られたものが
...
/-!
## 9. 証明検査の詳説 — エラボレータとカーネル {#sec-CH2.checking-details}
...
エラボレータの型推論は、[`Intro1a.lean` 3節](#sec-Intro1.functions)で見た2つの向き——
...
[4節](#sec-CH2.existence)の `isEven_double` を、証明を知らないつもりで
...
`⟨n, _⟩` と書けば、報告は `⊢ n + n = 2 * n` に変わる。[4節](#sec-CH2.existence)の梯子
...
1. `n + 1 - 1 = n` は（`n - 1 + 1 = n` と違って）すべての `n` で正しい。
   ...
   実際の値を渡す必要がある——[`Intro1a.lean` 4節](#sec-Intro1.dependent-functions)の括弧の読み分けの復習である。）
2. `#eval (3 : Nat) - 5` の値を予想してから確かめよ
   （本文の `0 - 1` と同じく、自然数の引き算は切り捨てである）。
-/

/-!
## 10. 対応のずれ {#sec-CH.prop-elimination}

対応は完全ではない。[`CH1.lean` 1節](#sec-CH.propositions)で見た**証明無関連性**により、
...
[3節](#sec-CH.empty-types)の `False.elim` や等式によるデータの書き換えは、その例である。
-/
...
`Prop` の世界から `Type` の世界へ渡れている。[3節](#sec-CH.empty-types)から使ってきた `.elim` や、
...
/-!
これで二つの証明編の仕事——対応を作り、検査の正体を見届けること——は
