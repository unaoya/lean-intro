...
`Ascoli.lean` の全問題の解答。問題文と説明はそちらを参照。
-/
...
/-- 問題10: 有限個の点 `P` ごとに、`H` を「`P` の各点で `W`-近い」集まり有限個で覆う（`P` についての再帰）。 -/
...
/-- 問題11（主定理・Ascoli の定理、全有界版）: 等連続かつ各点で全有界なら、一様収束について全有界。 -/
...
/-- `Option Nat` = 自然数に無限遠点 `none` を付け加えた空間。 -/
...
/-- 問題33（主定理・Ascoli の定理、コンパクト版）: 等連続・各点で相対コンパクト・閉なら、コンパクト。 -/
