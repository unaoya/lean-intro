...
`06_Topology.lean` まで読み終えた人のための演習問題。題材は次の5つ。
...
最後に、これらを使って **`06_Topology.lean` の主定理のハウスドルフという仮定が
外せないこと**を反例で確かめる。
...
* 解答は `07_ExercisesSol.lean` にある。
* 各宣言の直後の `#check` には、`06_Topology.lean` と同じ流儀で「表示」と「読み」を添えてある。

## 記法の準備

`06_Topology.lean` の `Continuous` は位相をインスタンス引数で受け取るので、
...
離散位相（`06_Topology.lean` の `discrete`、すべての部分集合が開）の対極として、
...
/-- 問題7: 忘却関手 `U : 06_Topology → Type` を作れ。位相を忘れて台の型だけを残す。 -/
...
/-- 問題8: 離散関手 `D : Type → 06_Topology` を作れ。型に離散位相を載せる。
...
/-- 問題9: 密着関手 `I : Type → 06_Topology` を作れ。型に密着位相を載せる。
射の行き先は問題3で作った。 -/
...
があること。全単射は `06_Topology.lean` の `Homeomorph` と同じ形の structure で表す。
...
`06_Topology.lean` の主定理は「コンパクト空間から**ハウスドルフ空間**への連続全単射は同相」。
...
片方は `06_Topology.lean` の `isOpen_of_nhds` がそのまま効く。
...
`06_Topology.lean` の補題2（選択を避けた）と対照的に、ここでは選択が本質的に要る。
...
`06_Topology.lean` の `[TopologicalSpace X]`）。この Part では、
...
なお、商位相の例では商型 `Quot` を使う。`05_MathematicalTools.lean` の末尾で
「商型はこの教材では使わない」と述べたのは本編（Intro・CH・06_Topology）の話で、
...
取る。同じものを、ここでは**上から**定義する:
    ...
    s が開 ⟺ すべての f i を連続にするどの位相 t' でも s は開
...
`X × Y` と書くだけで位相が自動で載るようになる——[`05_MathematicalTools.lean` 1節](#sec-Intro2.classes)で見た
class の効き目である。
-/
...
押し出しは離散になるが終域は密着）。ところが、`06_Topology.lean` の主定理と
...
`06_Topology.lean` では1ページかかった証明が、商写像の言葉では4行になる。 -/
