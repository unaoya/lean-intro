...
`Intro1.lean` で見た項の**作り方・使い方**は、命題の**証明の仕方・使い方**と対応する。
例えば関数は、入力の項を受け取って出力の項を作るものである。
一方、「P ならば Q」の証明は、P の証明を受け取って Q の証明を作るものとして読める。
だから、関数を作る操作は含意を証明する操作に、関数を適用する操作は含意を使う操作に対応する。

* 型 `α` に対しては、項 `a : α` を**作る**。
* 命題 `p` に対しては、証明 `h : p` を**書く**。

これは、別々の仕組みがたまたま似ているという話ではない。
Lean では命題そのものを型として扱うことで、項と証明を同じ規則で作り、使うことができる。
...
`.symm` で等式の左右を入れ替える。
`(Nat.two_mul n).symm` は `Eq.symm (Nat.two_mul n)` という関数適用の略記である。
証明も項なので、[`Intro1.lean` 5節](#sec-Intro1.structures)の `p.swap` と同じく、
その型を手がかりに関数を見つけて適用できる。
-/
...
`Prop` や `Type`、`Type 1` などをまとめて扱う宇宙の表記である。
いまは自然数についての適用を読むので、`α` や `β` に `Nat` を当てはめて追えばよい。
`Sort` 自体の読み方は[1節](#sec-CH.propositions)で整理する）:
...
インスタンスを探して補う仕組みは、[`Intro2.lean` 1節](#sec-Intro2.classes)で説明する。
ここでは等式や不等式が `Prop` 型の項であることが分かればよい。
`Eq` の定義そのものは[9節](#sec-CH.nat-proofs)で見る。
-/
...
証明をデータとして使う際の制約との関係は、[11節](#sec-CH.prop-elimination)で見る。
-/
...
`rfl` 自身の型も調べたいが、その表示を読むために、宇宙の表記を1つ整理しておこう。
-/
...
`Eq` とその構成子の定義は[9節](#sec-CH.nat-proofs)で見る。
...
[`Intro1.lean` 2節](#sec-Intro1.definitions)以来の検査が、そのまま**証明の検査**になっている。
...
例えば関数 `fun x => f x` と `f` の同一視や、先ほどの証明無関連性も含まれる。
...
証明項の言葉では、`h : p ∧ q` から `h.1 : p` と `h.2 : q` を作り、
それぞれ `p` の証明・`q` の証明として使える。
[`Intro1.lean` 5節](#sec-Intro1.structures)でフィールドを番号で指定したのと同じ記法である。
-/
...
エラボレータの型推論は、[`Intro1.lean` 3節](#sec-Intro1.functions)で見た2つの向き——
部分項の型を下から計算し、期待される型を上から伝える——を組み合わせて行われる。
数字 `2` を `Nat` と読むか `Int` と読むかが決まるのも、この段階である。
...
対応は完全ではない。[1節](#sec-CH.propositions)で見た**証明無関連性**により、
同じ命題の証明は、どの証明を選んだかによらず型検査で同一視される。
一方、`Type` 側のデータでは、例えば `Bool.true` と `Bool.false` のように
項の中身が区別される。この差が、**使うとき**に現れる。
