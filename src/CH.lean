...
まず[0節](#sec-CH.reading-proofs)で「そもそも証明するとは何をすることか」を具体例で考え直し、
**命題に対応する型の項を作ることが証明することだ**という、このファイルの
中心の主張を立てる。そのあと、すでにある型（命題）から新しい型（命題）を作る操作を6つ並べ、
それぞれについて、その命題を**示すとき**の書き方と**使うとき**の書き方を見比べる
（型の側では「項を作るとき」と「項を使うとき」にあたる。論理学で
導入則・除去則と呼ばれる区別だが、この教材では呼び名もこの2語で通す）。
...
このファイルで納得したいのは、次の一つの主張である:

**命題に対応する型があり、その型の項を作ることが、その命題を証明することである。**

これが納得できれば、話は一気に進む。証明であるべき項の型を検査して、それが
元の命題に一致していれば、**証明の正しさを機械的に確かめたことになる**——
検査の手段は、`Intro1.lean` の型検査そのものだからである。

だがこの主張は、初めて見ると奇妙に響く。証明とは論理をたどる営みで、項とは
`fun n => n + n` のような式だったはずである。なぜ式を作ることが証明になるのか。
それを納得するために、まず**そもそも証明する、証明を読んで納得するとは
何をすることか**を、考え直すところから始めたい。
...
以下、初等的な事実を5つ、この観点で書き直していく。それぞれについて
「ふつうの証明」から出発し、省略をすべて書き出した「詳細版」、
主張そのものを述語論理で書く「論理式」、各段で何の証明の仕方を
名指ししているかを言葉にした「形式化を意識した版」を経て、
最後に「Lean の項」に至る。例1・例2では算術の事実を既知の定理として使い、
例3ではその「既知」を信用せず自分で証明し、例4・例5では関数の性質を扱う。
...
（この「既知」を信用せず自分で証明するとどうなるかは、例3で掘り下げる）。

使った頭の動きは3つだけである: **全称の証明**（任意に取って示す）、
**定義に戻る**、**存在の証明**（候補+性質の証明）。

**項**。これを Lean に持ち込む。まず命題を書く記号を先取りする
（それぞれ後の節で主役になる）。命題は `Prop` 型の項として書けるのだった
（[`Intro1.lean` 1節](#sec-Intro1.terms-types)の先取りの回収である）。
...
定理の宣言は `theorem` で行う——**形は `def` とまったく同じ**で、
コロンの右の「型」の位置に命題を書き、`:=` の右に証明を書く
（`theorem` の正確な位置づけは次節で述べる）。その証明が、項である:
-/
...
* `fun n =>` が「n を任意に取る」——全称の証明は、**どの n を渡されても、
  ...
  `fun` そのもの）。
* `⟨n, …⟩` が「候補と、性質を満たす証明の組」——存在の証明は**組**になる
  （[`Intro1.lean` 5節](#sec-Intro1.structures)の `⟨ ⟩` そのもの）。なお、
  この候補のことを以後**証人**と呼ぶ。
* `(Nat.two_mul n).symm` が「既知の定理を使う」——既知の定理
  `Nat.two_mul n : 2 * n = n + n` は名前のついた完成品の項で、`n` を渡す
  **適用**の形で取り出す。`.symm` は等式の左右を入れ替える道具である。

番号にも注意——詳細版の 1→3→2 の順に部品が並び替わっている。
...
`match` をパターン `⟨k, hk⟩` で使えばよい——存在の証明は「証人と根拠の組」
だから、組と同じようにほどける。「両辺に同じ操作」は `congrArg`、「つなぐ」は
...
項では `fun` と同じ扱いである（詳細版の (1) は引数の宣言に吸収されている）。
...
ある。コメントの番号で詳細版と対応づけたので、読みにくければ番号の順に
内側から組み立て直しながら読むとよい。例1と同じく、**散文は発見の順、
項は命題の形の順**である。
-/
...
/-!
### 例3: 掘り下げ — 「既知」だった 2n = n + n を自分で証明する

例1で使った `Nat.two_mul` を、今度は信用せずに自分で証明してみる。

**ふつうの証明**。「n についての帰納法で明らか。∎」
...
theorem two_mul' : ∀ n : Nat, 2 * n = n + n := fun n =>
  match n with
  | 0     => rfl
  | k + 1 =>
    (congrArg (· + 2) (two_mul' k)).trans          -- 2(k+1) = 2k+2 = (k+k)+2
      ((congrArg (· + 1) (succ_add' k k)).symm :
        (k + k) + 2 = (k + 1) + (k + 1))           -- 型注釈は詳細版の式変形の写し

#check two_mul'

/-!
    two_mul' (n : Nat) : 2 * n = n + n
...
/-!
    'two_mul'' does not depend on any axioms

例1で「既知」と呼んだものは信用ではない——**底（定義と計算）まで、
すべて検査済みの項**なのである。
...
対象を当てはめて適用すること**——例1の「既知の定理 Nat.two_mul を n に適用」
と、実は同じ動きである。仮定も既知の定理も、文脈にある**部品**という資格は
変わらない。
...
そのものになる。詳細版で意識した**特殊化**（u := f x、v := f y）を、
ここでは引数として自分の手で書いている。

単射のような頻出の性質には、ライブラリが名前のついた述語を用意している——
`Function.Injective f` である（例1の `IsEven` と同じ、略記の発想）。
これで書き直すと:
-/
...
**形式化を意識した版**（今回は詳細版を兼ねる。ここまでの動きの総復習である）。
c を任意に取る（全称の証明）。全射の定義「どの点にも、そこへ送られてくる元が
ある」に戻ると、hg c は「g b = c となる b の存在」の証明——存在の使い方で
b と hb を**取り出す**（例2）。hf b からも a と ha を取り出す。示すべきは
「g (f a) = c となる a の存在」だから、候補 a と性質の証明を組にする（例1）。
...
例4と並べると対になっている: 単射は `fun` と適用だけの世界、全射は取り出しと
組の世界。そして入れ子の `match` は、「b を取り出した**あとの世界**で a を
取り出す」という証明の依存関係を、形として見せている。

### ここまでのまとめ — 頭の動きの一覧

5つの例で使った頭の動きは、これだけである。

| 頭の動き | 項での正体 |
|---|---|
| 全称を示す: 任意に取る | `fun n => …` |
| 全称を示す: 帰納法 | 再帰する `match` |
...
主張は論理式に（そして型注釈を付ければそのまま Lean の型に）、証明は
「ふつうの証明 → 詳細版 → 形式化を意識した版 → 項」へと磨かれた。
進むにつれて増えたのは**省略の穴埋め**だけで、新しい種類の思考は
何も足していない。形式化とは翻訳ではなく、**詳細化の極限**である。

証明を読んで納得するとは、冒頭のモデル化のとおり、有限種類の動きが正しく
行われていることを一文ずつ検査することだった。各動きは項の部品に写り、
その検査は**項の型検査**に写る。そして、詳細版の各文に読者が行う局所的な
照合——「文脈にあるか」「形が合うか」——を、一行も飽きず、一箇所も
「まあいいか」と流さずに実行するものこそ、型検査器である。冒頭の主張——
命題に対応する型の項を作ることが証明することである——は、この表を信じる
なら、もう奇妙には響かないはずである。

残りの節は、この表を精密化する。[1節](#sec-CH.propositions)で最小の例
（1 + 1 = 2）を解剖し、[2](#sec-CH.implication)〜[7節](#sec-CH.dependent-sums)で
「ならば・かつ・または・矛盾・すべての・存在する」を型の世界と1対1に対応させ、
...
2. 「A より B、B より C」の連鎖 `theorem chain {p q r : Prop} : (p → q) → (q → r) → p → r` を
   項で書け。
...
/-! ## 6. 依存積と「すべての」 {#sec-CH.dependent-products}
...
/-! ## 7. 依存和と「存在する」 {#sec-CH.dependent-sums}
...
1. `theorem exists_map {α : Type} {Q R : α → Prop} :
   (∀ a, Q a → R a) → (∃ a, Q a) → ∃ a, R a` を
...
ここまで「型検査」と一括りに呼んできた処理は、正確には2段階に分かれている。
...
`⟨n, _⟩` と書けば、報告は `⊢ n + n = 2 * n` に変わる。[0節](#sec-CH.reading-proofs)の4段の梯子は
**読む**ための道具立てだったが、**書く**ときは、この機械との対話で同じ梯子を
一段ずつ降りていける。
