...
`CH1.lean` では、関数を作り適用することで、ならば・全称の証明を読んだ。
この章では、項を作る道具を増やす。構成子で項を作り、場合分けや射影で使う仕組みを学ぶ。
次の `CH2.lean` では、それが「かつ」「または」「存在する」などの証明に対応する。
...
では、[`Intro1a.lean` 1節](#sec-Intro1.terms-types)で `Bool.true` を単に `true` と書けたのはなぜか。
...
（[`Intro1a.lean` 3節](#sec-Intro1.functions)で見た、期待される型を上から伝える仕組みの1例）。
...
詳しい型や使い方は、[`CH2.lean` 8節](#sec-CH.nat-proofs)で見る。
-/
...
この見方は [`CH2.lean` 7節](#sec-CH.introduction-elimination)で改めて説明する。
-/
...
[`CH2.lean` 3節](#sec-CH.empty-types)の `elimEmpty` で実物を見る。）

実は同じことが `Prop` の世界でもできて、命題の `True` と `False` は
帰納型として定義されている。命題の世界の帰納型の話は、このファイルでは
扱わず [`CH2.lean` 3節](#sec-CH.empty-types)でまとめて見る。

### 構成子は引数を取れる

構成子に引数を持たせることもできる。構成子の引数は、
`def` と同じくコロンの左に書ける（[`Intro1a.lean` 3節](#sec-Intro1.functions)の binder 形式）:
-/
...
証明の読み方は、[`CH2.lean` 8節](#sec-CH.nat-proofs)で扱う。
-/
...
binder 形式を読み替えれば `MySum : Type → Type → Type`——[`Intro1a.lean` 3節](#sec-Intro1.functions)の `Map` と
...
先頭に付いた。これは [`Intro1a.lean` の4節](#sec-Intro1.dependent-functions)で見た暗黙引数である。
...
[`Intro1a.lean` の4節](#sec-Intro1.dependent-functions)で学んだ依存関数として適用できる。
-/
...
`succ (succ (succ zero))` の表示）、`CH2.lean` に出てくる `×` `⊕`、
...
（これも[`Intro1a.lean` 3節](#sec-Intro1.functions)の「上から伝える」仕組みである）。
...
1. `#check Pair.mk true 0` の表示を予想してから確かめよ
   （`α`・`β` は何に決まるか）。
2. （発展）[`Intro1a.lean` 3節](#sec-Intro1.functions)の「カリー化」の正体を自分で書く。組を受け取る関数を
   「1つずつ受け取る」形に直す
...
[`Intro1a.lean` 1節](#sec-Intro1.terms-types)で見たとおり型も項だから、フィールドの値として**型そのもの**を持たせ、
...
これは[`Intro1a.lean` 1節](#sec-Intro1.terms-types)で見た `Type : Type 1` と対応している。
...
固定した行き先では書けない。これは [`Intro1a.lean` の4節](#sec-Intro1.dependent-functions)で学んだ依存関数型である。
-/
...
/-!
### 先取り（CH2）: 証明をフィールドに持つ構造

この「後の成分が前の成分に依存する組」の一般論は、[`CH2.lean` の6節](#sec-CH.dependent-sums)（依存和）で
...
8. （発展）集合 A の対角写像 Δ_A : A → A × A を、Δ_A(a) = (a, a) で定める。
   ...
   [`Intro1a.lean` 3節](#sec-Intro1.functions)の練習（`(Nat → Nat) → Nat` の項を2つ書く）と比較し、
   **型の形によって、書ける項の自由度がどう変わるか**を考えよ。
-/

/-!
関数に加えて、構成子と場合分けで項を作り使う道具がそろった。
次の `CH2.lean` で、存在を含む証明を読もう。その後は `Intro2.lean` → `Top.lean` と進む。
-/
