...
読む順は Intro1 → `CH.lean` → Intro2 → `Top.lean`。
このファイル（Intro1）は主に**目標1**を担当し、`CH.lean` を読むために
必要な最低限の言語機能だけを説明する。`Top.lean` のための残りの道具
（class・`Fin`・記法の自作など）は、`CH.lean` のあとの `Intro2.lean` で揃える。
...
型ごとに決まっている（仕組みは `Intro2.lean` で説明する）。ここでは `Nat` に対して
...
* `+` — **ざっくり**: `Nat → Nat → Nat` という型の2引数関数で、
  ...
  **正確には**: `+` の読み先は汎用の演算 `HAdd.hAdd` で、「どの型の足し算か」は
  登録簿（`Intro2.lean` の instance）から決まる。それを `Nat` に特殊化した姿がざっくり版である。
* `*` — `+` と同様の記法である（ざっくり `Nat → Nat → Nat` の2引数関数、
  正確には `HMul` という汎用演算の `Nat` への特殊化）。
* 数字 `3` — **ざっくり**: そのまま自然数の項。
  **正確には**: 数字も記法であり、期待される型に応じて読み方が決まる
  （仕組みと実例は `Intro2.lean` で見る）。何も指定がなければ `Nat` と読まれる。
-/
