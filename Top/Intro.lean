...
これがあるから「型を受け取る関数」「型を返す関数」が書ける（5節）。
-/

/-! ### ✏ 練習

1. 次のそれぞれについて、表示される型を**紙で予想してから** `#check` で確かめよ:
   `(3 + 4) * 2`、`Nat → Bool`、`3 < 5`。
2. `#check ((3 : Nat) = 5)` は通るか。通るなら型は何か。
   予想してから確かめよ（偽の命題も命題である、を思い出すこと）。
-/
...
/-!
`rfl` は「両辺が定義から計算して一致する」ことを理由にする等式の証明（詳しくは4節）。
...
1. `def five : Nat := 5` と `theorem two_add_three : 2 + 3 = 5 := rfl` を
   自分で宣言し、受理されることを確かめよ。
2. `theorem oops : 2 + 2 = 5 := rfl` は受理されるか。予想してから試し、
...
1. `#check` する**前に**型を計算せよ: `plus (double 3)`、`applyTo21 (plus 3)`、
   `fun n : Nat => plus n n`。それから確かめよ。
...
第二フィールドの型が第一フィールドの**値**に依存している。つまり `Fin n` は
「`n` 未満の自然数」を、値と証明の抱き合わせで表した型である。
このように、述語で切り出した「値と証明の組」の型を**部分型**と呼ぶ
（依存する直積としての一般論は `CH.lean` の6節で、部分型の記法は `Top.lean` で扱う）。
項は構成子で `⟨2, 証明⟩` のように作り、`.val` で値を取り出す——5節で実際に使う。
...
この節のパターンマッチに現れた `.red` や `.succ` もこの用法で、
...
1節の「正確には」で、「`+` の正体は汎用の演算で、どの型の足し算かは
...
（5節の補足で、`(2 : Fin 3)` のようなリテラルが通る理由として再登場する）。
...
3. 本文で `HasZero Bool` が未登録のため `inferInstance` が失敗する例を見た。
...
#check last
-- `⟨…⟩` は構成子に引数を渡す記法（4節）。`Fin` の項は「値」と「値 < n の証明」の組だった。
...
（`Fin` 用の読み方が instance として登録されている——4節で見た仕組みである）。
...
だから「structure は直積」（4節）と「帰納型は直和のようなもの」は矛盾しない。
...
1. 2点の列挙型 `inductive Two where | a | b` を定義せよ。`Two` と `Bool` は
