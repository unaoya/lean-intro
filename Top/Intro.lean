...
教材全体の目標はトップページに掲げた2つ——**目標1**（Lean を読めるようになる:
コードを読むとは項の型を推測することで、それは機械的に実行できる）と
**目標2**（その同じ仕組みが証明の検証に使えると納得する）——である。
このファイル（Intro）は主に**目標1**を担当し、項の型を推測するために
必要な最低限の言語機能だけを説明する。
...
各節末の ✏ 練習で手を動かすのも、書ける人になるためではない。短い定義を
自分で起こす問題もあるが、目的は同じ——**書いた項に、自分が予想した型が
付くかどうかを、機械に答え合わせさせる**ためである（コードを書く仕事は、
AI に任せてよい）。
...
* `*` — `+` と同様の記法である（ざっくり `Nat → Nat → Nat` の2引数関数、
  正確には `HMul` という汎用演算の `Nat` への特殊化）。
* 数字 `3` — **ざっくり**: そのまま自然数の項。
...
同じ手順で確かめられる。この後の節で新しい項の作り方（`match`・依存関数など）を
...
これが帰納型の項の使い方（除去）である。`fun` で受け取った `s` を、
...
なお、関数定義では `:= fun s => match s with` の部分をまるごと省略した
**関数方程式**の書き方も許されている。次はまったく同じ定義である:
...
4. 前問の `Nat` 版: `MyNat` と `Nat` も、型としては別物だが「同型」である。
   ...
   （`ofN` では、`Nat` の項をパターン `| 0 => …` と `| n + 1 => …` で
   場合分けできる——この書き方はここが初出である。
   また、**すべての** `n` で往復が恒等になることの証明には数学的帰納法が要る。
...
* `[inst : C α]` — **インスタンス引数**。これも使う側は書かないが、埋め方が違う。
  他の引数との単一化だけでは決まらず、エラボレータが `instance` として
  **登録された項**の中から探索して埋める。
