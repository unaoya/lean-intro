...
複雑な項の型も、部分項の型にこの規則を繰り返し当てるだけで決まる。
例えば `double (plus 3 4)` なら:

1. `3 : Nat`、`4 : Nat`（数リテラル）
2. `plus : Nat → Nat → Nat`（宣言でそう決めた）
3. 規則より `plus 3 : Nat → Nat`
4. もう一度規則より `plus 3 4 : Nat`
5. `double : Nat → Nat` だから `double (plus 3 4) : Nat`

どの段階でも、見ているのは**直下の部分項の型と規則1つだけ**である。
この局所的な計算を項全体に再帰的に行うのが型検査で、
