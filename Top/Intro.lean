...
`#check e` は項 `e` の型を表示する。まずは関数の出てこない、いちばん単純なところから。
-/
...
型と項はレイヤーの違う概念だが、**型を表す表記それ自体もまた項である**:
...
この教材では宇宙には**深入りしない**。階層があること、そして最後に出てきた
`Prop`——命題たちの住む宇宙（4節で使う）——があること、の2点だけ押さえれば足りる。
...
2. `#check (2 : Nat)` と `#check Nat` の表示を見比べ、どちらが「項の型」を、
   どちらが「型（を表す項）の型」を表示しているか、言葉にせよ。
-/
...
1. `def five : Nat := 5` を自分で宣言し、`#check` と `#eval` で確かめよ。
...
/-!
本体に出てきた `+` について、ここで一言だけ注意しておく。
`+` は **notation（記法）**であり、その意味は **class／instance** という仕組みで
型ごとに決まっている（仕組みそのものは7節で説明する）。ここでは `Nat` に対して
...
集合のアナロジーも添えておく。関数型 `A → B` は、集合の言葉では
「A から B への写像全体の集合」——Map(A, B)、Hom(A, B)、B^A などと
書かれるもの——に当たる。
-/
...
さきほどの `3 + 4 : Nat` も、ざっくり版の `+ : Nat → Nat → Nat` を使えば
...
* `=` `<` — **ざっくり**: `Nat` の上では、どちらも `Nat → Nat → Prop` という型の
  ...
  **正確には**: `=` はどの型でも使える `Eq : α → α → Prop`（`α` は両辺の型から
  決まる暗黙引数——6節の括弧で説明する。定義そのものは `CH.lean` の8節で見る）、
...
1. `#check 3 < 5` と `#check ((3 : Nat) = 5)` の表示を予想してから確かめよ
   （偽の命題も命題である、を思い出すこと）。
...
型を作る部品の1つ目が、帰納型である。**構成子（constructor）のリスト**で型を定義する。
...
`型名.構成子名` になる。作り方を列挙したら、それで型が決まる。
集合のアナロジーでは、列挙型は3点集合 {red, yellow, green} のような
**有限集合**である。
...
（この省略の仕組みは6節のドット記法で説明する）。以後は省略形も使う:
-/
...
まぎらわしいが、命題の `False` と `True` は `Bool` の項とは別物である。
こちらも Prelude で帰納型として定義されている:
...
見てのとおり、帰納型は `Type` に住む型だけでなく、**`Prop` に住む型＝命題**も
作れる。`True` は構成子をちょうど1つ持つ命題（構成子 `True.intro` が証明）、
`False` は構成子を**1つも持たない**命題（だから証明がない）。
`true : Bool` は計算で使う**データ**であり、`True : Prop` は**命題**である。
この対比は `CH.lean` の4節で扱う。

### 構成子は引数を取れる

構成子に引数を持たせると、データを包む型が作れる。構成子の引数も、
...
/-!
（矢印形式で `| nat : Nat → NatOrBool` と書いても同じである。）
`NatOrBool` の項は「`nat` の札が付いた自然数」か「`bool` の札が付いた真偽値」の
どちらか。場合分けで札を見分け、中身を取り出す:
-/
...
**直和（非交和）** α ⊔ β である（`CH.lean` に出てくる `⊕` は、
標準ライブラリにあるこれと同じ型）。
-/
...
`Bool`（上で引用した）、`Nat`（リテラル `3` は `succ (succ (succ zero))` の表示）、
...
#check Point.x     -- Point.x (self : Point) : Nat （自動定義された取り出し関数。`p.x` とも書ける）
...
名前空間（9節）に置かれた関数は、ドットを挟んだ短い書き方で呼べる。
...
1. `namespace Geometry … end Geometry` をもう一度開いて `unitX : Point := ⟨1, 0⟩` を
   ...
   見えないことを確かめよ。
2. `⟪1, true⟫` にならって、`Point` 用の記法（例えば `⟬x, y⟭`）を
