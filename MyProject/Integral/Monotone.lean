...
theorem integral_nonneg (f : Real → Real) (a b : Real)
  ...
  rcases hf with ⟨i, h'⟩
  rw [(IsIntegral_iff _ _ _ _).mp h']
  rw [IsIntegral] at h'
  false_or_by_contra
  rename_i h₁
  ...
  have h₁ : Δ.IsRepr a b n ξ := fun i => ⟨le_refl _, Δ.increase i⟩
  have h₂ : Partition.diam n a b Δ < δ := by
    ...
    · apply le_lt_trans' _ _ _ _ this
      ...
      exact hδ.1
    · exact hδ.1
  have h₃ : abs (RiemannSum f a b n Δ ξ - i) < ε := hδ.2 _ _ _ h₁ h₂
  have h₄ : 0 ≤ RiemannSum f a b n Δ ξ := RiemannSum_nonneg f a b n Δ ξ fnn h₁
