...
theorem pos_abs {x : Real} (h : 0 < x) : x.abs = x := by
  ...
  have hneg_le : -x ≤ 0 := by
    have := add_le_add_right (0 : Real) x (-x) h.1
    rw [zero_add', add_neg'] at this; exact this
  rw [if_neg (fun hle => h.2 (le_antisymm (0 : Real) x h.1
...
theorem abs_neg (z : Real) : Real.abs (-z) = Real.abs z := by
  ...
  | inl h =>
    rw [if_pos h]
    cases Classical.em (-z ≤ z) with
    | inl h' => rw [if_pos h']; exact le_antisymm z (-z) h h'
    | inr h' => rw [if_neg h']
  | inr h =>
    rw [if_neg h]
    rw [if_pos (by cases le_total z (-z) with
...
theorem abs_triangle (a b : Real) : abs (a + b) ≤ abs a + abs b := by
  ...
  · -- -(a+b) ≤ abs a + abs b
    ...
    exact LinearOrderedField.le_trans _ _ _
      (add_le_add_right (-a) (Real.abs a) (-b) (neg_le_abs a))
      (add_left_le (Real.abs a) (-b) (Real.abs b) (neg_le_abs b))
  · exact LinearOrderedField.le_trans _ _ _
      (add_le_add_right a (Real.abs a) b (le_abs a))
...
theorem abs_mul_nonneg {x y : Real} (hy : 0 ≤ y) : Real.abs (x * y) = Real.abs x * y := by
  ...
  | inr hyne =>
    cases le_total (0 : Real) x with
    | inl hx => rw [nonneg_abs (mul_nonneg x y hx hy), nonneg_abs hx]
    | inr hx =>
      have hnx : (0 : Real) ≤ -x := neg_neg_nonneg x hx
      have hxy_neg : x * y ≤ 0 := by
        have h1 : (0 : Real) ≤ (-x) * y := mul_nonneg (-x) y hnx hy
        rw [neg_mul] at h1
        have := add_le_add_right (0 : Real) (-(x * y)) (x * y) h1
...
theorem abs_sub_abs_le (a b : Real) : (a.abs - b.abs).abs ≤ (a - b).abs := by
  ...
  · -- -(|a| - |b|) = |b| - |a| ≤ |a - b|
    ...
    have h2 : b.abs - a.abs ≤ (b - a).abs := by
      have h1' := add_le_add_right _ _ (-a.abs) h1
  ...
  · -- |a| - |b| ≤ |a - b|
    ...
    have h1' := add_le_add_right _ _ (-b.abs) h1
...
theorem abs_sub_le_of_mem {P Q s t : Real}
  ...
  apply abs_le
  · rw [neg_sub]
    exact le_trans (add_le_add_right t Q (-s) htQ) (add_left_le Q _ _ hnegs)
  · exact le_trans (add_le_add_right s Q (-t) hsQ) (add_left_le Q _ _ hnegt)
