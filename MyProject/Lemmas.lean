...
theorem summation_smul (n : Nat) (f : Range n → Real) (c : Real) :
  ...
  induction n with
  | zero => simp [Sumation]; exact (mul_zero' c).symm
  | succ n ih => simp only [Sumation]; rw [ih]; exact (CommRing.left_distrib c _ _).symm
...
theorem addtive_summation (n : Nat) (f g : Range n → Real) :
  ...
  induction n with
  | zero => simp [Sumation]; exact (zero_add' 0).symm
  | succ n ih =>
    simp only [Sumation]; rw [ih]
    exact add_four_comm _ _ _ _

theorem summation_congr (n : Nat) (f g : Range n → Real) (h : ∀ i, f i = g i) :
  Sumation n f = Sumation n g := congrArg _ (funext h)

theorem neg_summation (n : Nat) (f : Range n → Real) :
  -Sumation n f = Sumation n (fun i ↦ -f i) := by
  induction n with
  | zero => simp [Sumation]; exact neg_zero
  | succ n ih => simp only [Sumation]; rw [neg_add_distrib, ih]
...
theorem sumation_nonneg (n : Nat) (f : Range n → Real) (h : ∀ i, 0 ≤ f i) :
  ...
  | succ n ih =>
    ...
    have h1 : (0 : Real) ≤ Sumation n f' :=
      ih f' (fun i => h ⟨i.val, Nat.lt_of_lt_of_le i.property (Nat.le_succ n)⟩)
    have h2 := h ⟨n, Nat.lt_add_one n⟩
    have h3 := LinearOrderedField.add_le_add (0 : Real) (Sumation n f') (0 : Real) h1
    rw [add_zero'] at h3
...
theorem summation_le (n : Nat) (f g : Range n → Real) (h : ∀ i, f i ≤ g i) :
  ...
  | succ n ih =>
    ...
    have ih' : Sumation n f' ≤ Sumation n g' :=
    ...
    exact le_trans
      ...
      (add_left_le (Sumation n g') _ _ hn)
...
theorem summation_succ (n : Nat) (f : Range n.succ → Real) :
  Sumation n.succ f = Sumation n (fun i ↦ f (incl i)) + f ⟨n, by exact Nat.lt_add_one n⟩ := rfl
...
theorem summation_all_zero (n : Nat) : Sumation n (fun _ : Range n => (0 : Real)) = 0 := by
  induction n with
  | zero => rfl
  | succ m ih =>
    show Sumation m (fun _ : Range m => (0 : Real)) + (0 : Real) = 0
    rw [ih, zero_add']
...
theorem summation_split_term (n : Nat) (k : Range n) (f : Range n → Real)
    ...
    Sumation (n + 1) g = Sumation n f := by
  induction n with
  | zero => exact absurd k.property (Nat.not_lt_zero _)
  | succ n ih =>
