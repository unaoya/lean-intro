-- Text/C07_Tactics.lean — Ch7 タクティク入門（tactic mode の機構と信頼）
-- by・ゴール状態・intro/exact/apply/rw/have/show を学び、「タクティクは証明項を書く
-- 機械」であることを #print で確かめる。等式補題を数本だけ手で証明して道具に慣れる。
-- 等式コーパス本体は Ch8（C08_Rewrite）、自動化は Ch9、順序と calc は Ch10。
import Text.C06_Structures

-- （One Real bridge は C03 へ前倒し済み＝加群のスカラー単位 1 を Ch6 で使うため）

-- ============================================================
-- タクティク = 導入/除去規則を「ゴールから逆向きに」適用する機械
--   Ch1/Ch5 で見た導入/除去規則は、タクティクとして現れる（新しい原理ではない）:
--     intro           ＝ →/∀ の **導入**（`fun` を作る）
