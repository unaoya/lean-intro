...
- コード: C05_Summation.lean（Range・Summation・Partition・RiemannSum・IsRepr・leftRepr・trivialPartition。**加群は次章 Ch6 へ分離**）
...
## 5.6 帰納型を使う — 除去規則（recursor）と cases／induction
...
- **縦糸**: 「定義する再帰」と「証明する帰納」は同じ recursor。ergonomic な `cases`/`induction` タクティクは Ch7 以降、Σ 補題の本格コーパスは **Ch11**。Ch1（∨ の cases）→ Ch5（Nat の induction）→ Ch11（タクティク＋コーパス）で除去規則の糸を張る
- ⚠ Ch1–5 は term mode 基調。ここは `Nat.rec` による term mode の除去で `by` を使わない

## 5.7 証明を運ぶレコード — structure Partition
...
## 5.10 記法の自作 — notation 初登場

- Σ 記法とリーマン和の記法を「定義したらすぐ」自作する（メタプログラミング導入の第 1 段）

## 5.11 伏線回収: And も structure だった
