# Ch7 書き換えと 2 つの等しさ — rw と defeq

<!-- 下書き（配置設計版）: 節立てと内容の配置メモのみ。本文未執筆 -->
<!-- 2026-06-15 新設: Ch6 の等式コーパス＋旧 defeq 章の rw/defeq を統合（1章1テーマ） -->
...
- コード: C07_Rewrite.lean（等式/環コーパス本体: 消去・ゼロ・符号・引き算の整理・telescope_2）

## 7.1 等しさは 2 種類ある

- `a - b = a + -b := rfl` が通る——defeq（計算で同じ・カーネルが畳む）。だが rw は構文（見た目が同じ部分項）しか書き換えない
- show による defeq の言い換え（Ch6 の `show …` の正体）。「sub は a + -b の略記」を実感する。🪟 窓: 正規化と `#reduce` — 証明の簡約
...
- 群・環の恒等式を rw/calc のドリルとして獲得: add_left_cancel'（Ch6 既出）を足場に、mul_zero'・zero_mul'・neg_zero・neg_neg・neg_mul・mul_neg・neg_add_distrib・mul_sub
- 引き算の整理（sub は a + -b）: sub_self・sub_zero・neg_sub・add_sub_cancel 系・add_sub_add・mul_sub_mul・add_four_comm
- ボス: telescope_2（`b − a = (c − a) + (b − c)`＝Ch10 の望遠鏡和の部品）・add_half_sub_full（E 部の誤差ずらし）
