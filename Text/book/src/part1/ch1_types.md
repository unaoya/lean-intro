...
書くべきこと。
型をチェックする。
型のチェックが最大の機能で、これが証明を検証するためにも使える。
#checkコマンドを使ってみる。
#check Nat
#check 0
#check 0 = 0
...
- `example`: 名前を付けずに「その型の項が作れる」ことだけを示す（使い捨ての確認・以後多用する）。型のチェックだけしたいなど。:=の後の項が前の型かどうか、（証明が通っているかどうか）をチェックできる。引数を渡すこともできる。
...
- **Empty**: 構成子が 0 個——項が 1 つも作れない型。空集合と思える。どのような型にも関数を作れる。Curry-Howard 対応で命題 `False` に対応する（「あり得ない前提から何でも導ける」＝爆発律は `Empty.elim` として現れる）
...
メモ：型を集合と思うなら、積は集合の直積にあたる。Prodの型をチェックしよう。これは二つの型から一つの型を作る、という型を持っている。射影を二つ持っている。Prod.fstやProd.sndの型にも注目しよう。これらも型を引数にもつ。implicit argumentの例にもなっている。a : \alpha, b : \betaならProd.mk a bと書いた時点でProd.mk \alpha \beta a bになることが推測できる。なので暗黙の変数とできる。
...
メモ：直和からの関数は二つの関数の組みである。これがinductive typeのrecursorである。
集合の直和と思える。自然な埋め込みがある。これがSum.inlとSum.inrである。
直和からの射はそれぞれからの射を定めることに対応する（普遍性）。
これはパターンマッチ記法で書くことができる。あるいはSum.rec
...
メモ：def hoge (a : \alpha) : \beta := ...でも関数型の定義になる。引数を書いた形は関数型と解釈する。xがa型でない時、hoge xは型チェックを通らずエラーになる。hogeのdefを見ると、hoge xはxが\alpha型であるときのみ意味を持つことになる。（勝手にcastしてくれる場合もある、それは特殊ケース）
...
メモ：述語は命題の族と思える。そうすると依存和の特別な場合がSubtypeであるように見えてくる。
