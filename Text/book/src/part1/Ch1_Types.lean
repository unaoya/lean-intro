...
-- core にある基本的な型（空の型 Empty・数の型 Nat/Int/Rat）を見たあと、型を「作る」基本手段
-- ——積・和・関数・依存関数・部分型・依存和——を並べ、それぞれの
...
-- ============================================================
-- §0 Lean の基本コマンド: example・def・theorem・#check・#print
--   本書で繰り返し使う 5 つのコマンドをここで一度に押さえる。
-- ============================================================
...
example : Nat := 42
example : Nat → Nat := fun n => n + n

-- `theorem`: 命題（型）の証明（項）に名前を付ける。実体は def と同じ（型が命題＝Prop である def）。
...
-- ============================================================
-- 基本的な型: 空の型 Empty・数の型 Nat/Int/Rat（標準ライブラリに最初からある型）
--   型を「構成する」前に、core にある型をいくつか見る。
-- ============================================================
...
-- ============================================================
-- `inductive` キーワードで帰納型を自作する（Three・Nat の succ など）話と、その除去規則
--   recursor・#print 種明かしは Ch4（帰納型と再帰）でまとめて扱う。上の積・和・Subtype・Σ も
--   内部は帰納型だが、本章では「既存の型の構成」として使い分ける。
-- ============================================================
