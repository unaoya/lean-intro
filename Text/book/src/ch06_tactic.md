...
- 到達点: 基本タクティクで短い証明が書ける。「なぜタクティクで証明になるのか（タクティクは項を書く機械）」に答えられる・**各基本タクティクが Ch1/Ch4 の導入/除去規則に対応すると分かる**
- 新しい Lean 機能: by・ゴール状態・intro/exact/apply/have/show・term↔tactic の往復・One bridge・**タクティク↔導入/除去規則の対応**
- コード: C06_Tactics.lean（One bridge＋**タクティク↔規則（ANCHOR `tactics_as_rules`）**＋公理射影＋add_left_cancel'。等式コーパス本体は Ch7）
...
## 6.4 種明かし: タクティク = 導入/除去規則を逆向きに（ANCHOR `tactics_as_rules`）

- by 証明を `#print` して生成された λ 項を見る。**各タクティクは Ch1/Ch4 の導入/除去規則に対応**——新しい原理ではなく、規則を「ゴールから逆向きに」当てる機械:
  ...
  - `rfl` ＝ `=` の導入（Eq.refl）／`rw` ＝ `=` の除去（Eq.rec・Ch7 で深掘り）
