-- Text/Proto/InsertBound.lean — M5(iii)-B: 点挿入によるリーマン和の変化の評価
-- 脱 abs: |RS' − RS| ≤ 2M·len の代わりに両側述語 NearLe を導入し、
-- 2*M は M + M で書く（リテラル 2 不使用の設計方針）
import Text.Proto.Insert
...
--    （参照実装の |RS' − RS| ≤ 2M·len k の脱 abs 版。
--      有界性も両側 hMlo / hMhi で持つ）
-- ============================================================
...
theorem rs_insert_bound (f : Real → Real) {u v : Real} (c M : Real)
  ...
  have hc_v : c ≤ v :=
    calc c ≤ Δ.points (addone k) := hR
      ...
      _ = v := Δ.right
  have hξmem := tag_mem (⟨n, Δ, ξ, hr⟩ : TaggedPartition u v) k
...
--    m · ((M+M)·δ) で制御される（参照実装の diam 版の置き換え）
-- ============================================================
...
theorem rs_multi_insert_bound (f : Real → Real) {u v : Real} (M : Real)
  ...
  | succ m ih =>
    ...
    · -- 細かさは ∀ 形のまま保たれる（M5(iii)-A の insertPoint_fine）
...
#print axioms rs_insert_bound
#print axioms rs_multi_insert_bound
