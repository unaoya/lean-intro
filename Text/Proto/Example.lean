-- Text/Proto/Example.lean — M7: 貫通する具体例 y = x
--   sum_id: Σ_{i<n} i = n(n−1)/2 を (1+1)·Σ = n·(n−1) の形で（リテラル 2 不使用）
--   isintegral_id: f = id は定義から直接可積分（中点和が望遠鏡和になる稀有な例）
--   integral_id: 橋を 1 回かけて ∫ x = (b²−a²)/2
import Text.Proto.Main
...
-- 端点と中点の距離は半区間（rw のパターン捕獲を避けるため独立補題にする）
...
-- ============================================================
-- §2 sum_id: (1+1)·Σ_{i<n} i = n·(n−1)（Ch8 名物演習の素材）
-- ============================================================
...
-- ============================================================
-- §3 退化区間の RS は 0（[u,u] では全分点が u に潰れる）
-- ============================================================
...
-- ============================================================
-- §4 isintegral_id: 中点和の望遠鏡和による直接証明
...
theorem isintegral_id (a b : Real) (hab : a ≤ b) :
  ...
  | inr hne =>
    ...
    rw [← hmid_sum]
    have hnle : NearLe (ε / (1 + 1))
        (Summation P.n (fun i =>
          (P.Δ.points (incl i) + P.Δ.points (addone i)) / (1 + 1) * P.Δ.length i))
        (P.sum (fun x => x)) := by
      rw [hsum_eq]
      exact nearle_of_add hsum_lo hsum_hi
    exact nearle_to_near hnle (half_lt hε)

-- 橋をもう 1 回: Integral 関数の値として
