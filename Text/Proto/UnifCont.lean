-- Text/Proto/UnifCont.lean — M5(i): 閉区間上の連続関数の一様連続性と有界性
-- 参照実装の |·| ベースの証明を Near / NearLe の両側述語で書き直す（脱 abs）。
-- min も使わない: 「両方より小さい正数の存在」(exists_min2) で合流させる。
-- 一様連続性は sup 公理による連結性論法（S(t) = 「[u,t] 上 ε-一様連続」の sup が v に達する）。
import Text.Proto.Refine
import Text.Proto.Criterion
