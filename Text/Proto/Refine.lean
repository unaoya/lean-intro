-- Text/Proto/Refine.lean — M5(iii)-C: 細分比較（σ 写像とエンベロープ）
-- 固定した分割 Δ の全分点を任意の細かい分割 Δ' に挿入して共通細分 Δp を作り、
--   ・細分上で親の代表点を使った RS は元の RS に等しい（rs_refine_eq、ステップ原始関数）
--   ・挿入誤差は InsertBound の評価で θ 以下に抑えられる
-- を合成して「十分細かい 2 つの分割の RS は近い」（rs_compare → コーシー条件の素材）を得る。
--
-- 注意: rs_refine_eq の証明装置 stepAnti は min を使う。Proto では min を素朴な if で
-- 定義する（rmin）。この if は Classical.propDecidable を呼ぶため、これを使う定理の
-- 公理監査には choice が現れる（設計書の「素朴定義実験」の実例）。
import Text.Proto.InsertBound
...
-- ============================================================
-- §4 同一分割・別タグの RS の差: NearLe (C·(v−u))
-- ============================================================
...
-- ============================================================
-- §5 ステップ原始関数（証明内部の装置。rmin の if が choice の源）
-- ============================================================

-- 素朴な if による min（Classical.propDecidable 経由 → 監査に choice が出る）
...
-- ============================================================
-- §6 細分上で親区間の代表点を使った RS は元の RS に等しい
-- ============================================================
...
-- ============================================================
-- §7 Δ の全分点が Δp の分点に含まれるなら、Δp の各小区間は
...
-- ============================================================
-- §8 エンベロープ: 固定した Δ に対し、十分細かい任意の P' の RS は
...
-- ============================================================
-- §9 一様連続性（仮定）の下での 2 分割比較:
--    細かい P を固定すると、十分細かい任意の P' の RS は
--    ε'·(v−u) + θ の範囲に入る（コーシー条件の素材）
-- ============================================================
...
theorem rs_compare (f : Real → Real) {u v : Real} (M : Real)
  ...
  intro j
  have hsmem := tag_mem (⟨n, Δ, ξ, hr⟩ : TaggedPartition u v) (σ j)
  have htmem := tag_mem (⟨N, Δp, ξp, hrp⟩ : TaggedPartition u v) j
  have hb1 := hrp j
  have hb2 := hr (σ j)
...
#print axioms rs_refine_compare
