...
`13_FundamentalGroupoid.lean` の全問題の解答。主張は問題ファイルと一字一句同じにしてある。
-/
...
/-- 問題1: `1/2 ≤ s` なら貼り合わせは後半の値。境目 `s = 1/2` で両者が一致することを使う。 -/
...
/-- 問題3: **再パラメータ化の補題**: 端点で一致する `φ, ψ : [0,1] → [0,1]` について、`γ ∘ φ` と `γ ∘ ψ` はホモトピック。 -/
...
/-- 問題4: 写像を合成してから貼り合わせても、貼り合わせてから合成しても同じ。 -/
...
/-- 問題5: 前半への縮小 `halfL` で貼り合わせを読むと、前半の写像になる。 -/
...
/-- 問題17: 差が整数で、どちらも `a` から `1/2` 未満の距離にある 2 数は等しい。 -/
...
/-- 問題19: （発展）`a` のまわりの `1/2`-近傍の像は均等に被覆される。 -/
...
/-- 問題21: **デッキ変換**: 整数 `m` から持ち上げると、`0` から持ち上げた終点を `m` だけずらした点に着く。 -/
...
/-- 問題26: **主定理 π₁(S¹) ≃ ℤ**: モノドロミー次数は、連接を和に写す全単射である。 -/
theorem pi1_circle :
