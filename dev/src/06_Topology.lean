...
数学でいう「集合」には、`05_MathematicalTools.lean` で作った `Set` を使う
...
`01_TypesAndTerms.lean` と同じ Infoview 風の枠で書き添えてある。
`01_TypesAndTerms.lean` 以来の読み方——表示を見る前に
...
集合を返す関数」である（[`01_TypesAndTerms.lean` 4節](#sec-Intro1.dependent-functions)の型の族の仲間）。`sUnion` が
...
（[`01_TypesAndTerms.lean` 3節](#sec-Intro1.functions)の binder 形式）。`∀` と `→` を並べて書いても同じ型である。 -/
...
証明は `by` のタクティク（[`06_Topology.lean` 1節](#sec-Top.tactics)）。まず `⋃₀ ∅ = ∅` を `Set.ext` で示し、
...
いつでも展開してよい」という指定。`07_Exercises.lean` の instance 探索で効いてくるだけ
なので、ここでは気にしなくてよい。 -/
...
/-! 余力があれば、発展演習 `07_Exercises.lean` へ（解答は `07_ExercisesSol.lean`）。
