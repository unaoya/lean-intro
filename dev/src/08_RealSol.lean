...
`08_Real.lean` の全問題の解答。主張は問題ファイルと一字一句同じにしてある。
-/
...
/-- 問題20: （発展）**整数部分**: どの実数 `x` にも、`n ≤ x < n + 1` となる整数 `n` がある。 -/
...
/-- 問題23: 実数値関数の連続性は、各点で「値が `ε` 以内に収まる開近傍がある」で判定できる。 -/
...
/-- 問題28: **閉区間の連結性**: 閉区間を、互いに交わらない 2 つの開集合で分けることはできない。`a` を含む側が区間全体を含む。 -/
...
/-- 問題30: **ルベーグ数の補題**: 閉区間を覆う開集合族には、ある `δ > 0` があって、区間のどの点のまわりの `δ`-近傍も、族のどれか 1 つに収まる。 -/
