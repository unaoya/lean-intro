...
型の入門と証明編を2往復し、`04_Exists.lean` を読み終えた人のためのファイルで、
`06_Topology.lean`（位相空間）を読むのに必要な残りの道具を揃える:
...
[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)の `Pair (α β : Type)` のように
...
[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)の `PointedType` は、「型 `carrier` と、
...
`Sort u` は、[`02_Forall.lean` 0節](#sec-CH.propositions)で見た、`Prop` や `Type`、`Type 1` などを
...
[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)で作った `Point` に基点（原点）を登録してみる。
-/
...
試しに、[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)の `Pair` のための記法を作ってみる。
-/
...
（[`02_Forall.lean` 0節](#sec-CH.implication)の世界そのもの）である。名前を `Set.〜` にしたのは
ドット記法（[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)で見た関数適用の略記）のためで、
この命名の仕組みは次節で説明する。
...
[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)のドット記法や、前節の `Set.subset_refl` という名前を
