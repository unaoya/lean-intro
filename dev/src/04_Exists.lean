...
偶数の和、全射の合成を例に存在の証明を読み、同じ節で依存和との対応を整理する。
...
[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)でフィールドを番号で指定したのと同じ記法である。
-/
...
一点側の対応物もある。`Unit`（構成子1つ・引数なしの型、[`03_InductiveTypes.lean` 1節](#sec-Intro1.inductive-types)）に
対応する命題が `True` である:
...
/-!
## 4. 存在と依存和 — 偶数・全射の例から {#sec-CH2.existence}
...
理由は[8節](#sec-CH.prop-elimination)で見る。
...
証明も項なので、[`03_InductiveTypes.lean` 2節](#sec-Intro1.structures)の `p.swap` と同じく、
...
**項**。「取り出して名前を付ける」は、[`03_InductiveTypes.lean` 1節](#sec-Intro1.inductive-types)の
...
`Sort` 自体の読み方は[5節](#sec-CH.nat-proofs)で整理する）:
...
データとして第一成分を射影する操作との違いは、[8節](#sec-CH.prop-elimination)で詳しく見る。
-/
...
/-!
## 5. 等式と自然数の証明 — 計算・帰納法・構成子 {#sec-CH2.computation}
...
定義された命題であり（[1](#sec-CH.products)〜[3節](#sec-CH.empty-types)、[4節](#sec-CH.dependent-sums)）、示すときに使った `⟨_, _⟩` や `.inl` は、
それらの構成子だったのである。
...
/-!
## 6. 示すときと使うときのまとめ {#sec-CH.introduction-elimination}
...
| 依存和 | `(a : α) × P a` | `∃ a, Q a` | `⟨a, b⟩` | `.1` / `.2`（`∃` は `match` のみ——[8節](#sec-CH.prop-elimination)） |
...
/-!
## 7. 証明検査の詳説 — エラボレータとカーネル {#sec-CH2.checking-details}
...
ここでスローガンをより正確に言い直そう。
    ...
    ソースコード
      ...
      → エラボレータ: 記法を展開し、省略された引数（暗黙引数・インスタンス引数）を
        埋め、タクティク（[`06_Topology.lean` 1節](#sec-Top.tactics)）を実行して、省略のない項を組み立てる
...
エラボレータの型推論は、[`01_TypesAndTerms.lean` 3節](#sec-Intro1.functions)で見た2つの向き——
...
1. `n + 1 - 1 = n` は（`n - 1 + 1 = n` と違って）すべての `n` で正しい。
   ...
   実際の値を渡す必要がある——[`01_TypesAndTerms.lean` 4節](#sec-Intro1.dependent-functions)の括弧の読み分けの復習である。）
2. `#eval (3 : Nat) - 5` の値を予想してから確かめよ
   （本文の `0 - 1` と同じく、自然数の引き算は切り捨てである）。
-/

/-!
## 8. 対応のずれ {#sec-CH.prop-elimination}

対応は完全ではない。[`02_Forall.lean` 0節](#sec-CH.propositions)で見た**証明無関連性**により、
...
等式によるデータの書き換え（`▸`、`06_Topology.lean` で使用）が正当なのはこのためである。
-/
...
（タクティク）があり、`06_Topology.lean` の実戦ではそちらが主体になる。
