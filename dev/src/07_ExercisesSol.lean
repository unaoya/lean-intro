...
`07_Exercises.lean` の全問題の解答。問題文と説明はそちらを参照。
各宣言の直後の `#check` の注記は `06_Topology.lean` と同じ流儀。
-/
...
つまり同相ではない。`06_Topology.lean` の主定理と見比べると、
...
（終域の位相は押し出し位相に一致する）。
`06_Topology.lean` の補題1〜3の連鎖（閉→コンパクト→像コンパクト→像閉）を再利用する。 -/
