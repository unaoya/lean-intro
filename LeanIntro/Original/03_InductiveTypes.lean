...
/- ✏ 練習
40. 上の `match` の対象を `Signal.yellow` に変えた項と、`Signal.green` に変えた項を
...
/- ✏ 練習
41. `Signal.red`・`Signal.yellow`・`Signal.green` をそれぞれ `0`・`1`・`2` に
...
/- ✏ 練習
42. `Signal` の「逆回り」`prev : Signal → Signal` を `match` で定義し、
   `#eval prev (Signal.next Signal.red)` の表示を予想してから確かめよ
   （`Signal.red` に戻ってくるはずである）。
43. `isRed` にならって `isGreen : Signal → Bool` を書き、
   `#eval isGreen Signal.red` の値を予想してから確かめよ。
44. `def stopSignal : Signal := .red` を宣言し、`#check stopSignal` と
...
/- ✏ 練習
45. 中身の値には触れず、`.nat` の札なら `true`、`.bool` の札なら `false` を
   返す `tagOf : NatOrBool → Bool` を書け。不要な中身には `_` を使える。
   `#eval tagOf (NatOrBool.nat 3)` の値を予想してから確かめよ。

46. `#eval valueOf (NatOrBool.bool true)` の値を予想してから確かめよ
   （どちらの場合に当たるか）。
47. 「`bool` の札なら中身を、`nat` の札なら `false` を返す」関数
...
/- ✏ 練習
48. 自然数の組 `(a, b)` を表す型 `NatPair : Type` を `inductive` で定義せよ。
   構成子は `mk` の1つとし、自然数を2つ受け取るものとする。
   `#check NatPair.mk` と `#check NatPair.mk 3 5` の表示を予想して確かめよ。
49. 第1成分を返す写像 `firstNat : NatPair → Nat` と、第2成分を返す写像
...
50. 自然数 `n` と真偽値 `flag` の組を表す型 `FlaggedNat : Type` を
   `inductive` で定義せよ。構成子は `mk` の1つとする。
   `#check FlaggedNat.mk` と `#check FlaggedNat.mk 3 true` の表示を予想して確かめよ。
51. 自然数の成分を返す `numberOf : FlaggedNat → Nat` と、真偽値の成分を返す
...
/- ✏ 練習
52. 次の3種類のデータを表す型 `MixedData : Type` を `inductive` で定義せよ。
   構成子 `pair` は自然数を2つ、`flagged` は自然数と真偽値を受け取り、
   `empty` は引数を受け取らないものとする。
   3つの構成子の型を予想して `#check` で確かめよ。
53. 写像 `readNumber : MixedData → Nat` を、`pair a b` は `a + b` に、
...
/- ✏ 練習
54. `MySum Bool Bool` の左右どちらの札からも、中身の `Bool` をそのまま返す
...
/- ✏ 練習
55. `#eval getLeft 7 (MySum.inl 3 : MySum Nat Bool)` の値を予想してから
...
62. （発展）上で書いた `toN : MyNat → Nat` の逆向きの関数
...
/- ✏ 練習
63. 型 `A` と2本の写像 `f g : A → Nat` が与えられたとする。
...
64. `Point` の第1成分だけを1増やす `moveRight : Point → Point` を書け。
...
/- ✏ 練習
65. `Point.zeroX (p : Point) : Point` を定義し、第1成分だけを `0` にせよ。
   本体では `p.y` と `.mk` の両方を使う。`#eval (Point.mk 1 2).zeroX.x`
   の値を予想して確かめよ。
66. `#eval (Point.mk 1 2).swap.swap.x` の値を予想してから確かめよ
   （2回入れ替えると元に戻るはずである）。またこの式を、ドットを使わず
   フルネームの適用だけで書き直すと何になるか、紙に書いてから `#eval` で
   一致を確かめよ。

67. 次の structure を定義し、`#check` で構成子と取り出し関数が
   自動定義されていることを確かめよ:
...
72. 長方形から幅と高さの積を返す写像 `rectArea : Rect → Nat` を書け。
...
/- ✏ 練習
73. `#check Pair.mk true 0` の表示を予想してから確かめよ
   （`α`・`β` は何に決まるか）。
74. （発展）`01_TypesAndTerms.lean` 3節の「カリー化」の正体を自分で書く。組を受け取る関数を
   「1つずつ受け取る」形に直す
...
/- ✏ 練習
75. `def pointedBool : PointedType := ⟨Bool, true⟩` が受理されることを確かめよ。
   また `#check PointedType.mk Nat` の表示を予想してから確かめよ
   （第1引数を渡すと、第2引数の型が決まる）。

76. 自然数 n を点付き集合 (Nat, n) に送る写像 `attachNat : Nat → PointedType` を書け。
   型を `#check` し、`#reduce (attachNat 3).point` の値を予想して確かめよ。
77. 点付き集合 (A, a) から台となる型 A を取り出す写像 `baseType : PointedType → Type` を書け。
...
78. 点付き集合 (A, a) と写像 f : A → A から、点付き集合 (A, f(a)) を作る関数
...
/- ✏ 練習
79. 点付き集合 (A, a) から点 a を取り出す関数 `getPoint (p : PointedType) : p.carrier` を書け。
...
/- ✏ 練習
83. `#check (last 4).val` の表示を予想してから確かめよ
   （表示に付く `↑` は**強制**の印——上の補足で説明した）。
84. `#check first 4` の表示と、`#eval (first 4).val` の値を予想してから確かめよ。
85. `#check first 0` の表示を予想してから確かめよ（`Fin (0 + 1)` は1点の型である）。
-/

/- ✏ 練習
86. （補足の確認）`#eval (5 : Fin 4)` の表示を予想してから確かめよ。
87. （補足の確認）`#eval (first 5).val + (2 : Fin 3).val` の値を予想してから確かめよ。
-/
...
/- ✏ 練習
88. 写像 F : ℕ × ℕ → ℕ に対して、G(a, b) = F(b, a) で定まる写像
...
89. 写像の集合の間の写像 T : Map(ℕ, ℕ) → Map(ℕ, ℕ) を、
   ...
   信号の変化を3回たどってから確かめよ。
90. 2点集合 B = \{true, false} と3点集合 S = \{red, yellow, green} を考える。
...
93. 各集合 A に対して、その要素 a を点付き集合 (A, a) に送る写像を考える。
...
94. （発展）集合 A と写像 F : A → A に対して、その反復 F^n : A → A を、
