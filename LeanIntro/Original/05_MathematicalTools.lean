...
123. 本文で `Pointed Bool` が未登録のため `inferInstance` が失敗する例を見た。
...
127. `Add` にならって `instance : Mul Point where mul p q := ⟨p.x * q.x, p.y * q.y⟩`
...
/- ✏ 練習
128. `⟪1, true⟫` にならって、`Point` 用の記法（例えば `⟬x, y⟭`）を
   `syntax` と `macro_rules` で自作し、`#check ⟬1, 2⟭` で確かめよ。
129. `infixl:65 " ⊞ " => add` で、`03_InductiveTypes.lean` の `add`（`MyNat` の足し算）に
...
/- ✏ 練習
134. `namespace Geometry … end Geometry` をもう一度開いて `unitX : Point := ⟨1, 0⟩` を
