...
2. `theorem dni {p : Prop} : p → ¬¬p` を書け（`¬¬p` を**示す**向き。ほとんど1語で書ける）。
   ...
   書き方は `Top.lean` の2節 `compl_compl`（`Classical.byContradiction` を使う）で、
...
2. 2段階で根拠を移す
   ...
   を書け。証人は変えず、`CH1.lean` の含意の推移を各点で使えばよい。
-/
...
4. `#check first 4` の表示と、`#eval (first 4).val` の値を予想してから確かめよ。
5. `#check first 0` の表示を予想してから確かめよ（`Fin (0 + 1)` は1点の型である）。
6. （補足の確認）`#eval (5 : Fin 4)` の表示を予想してから確かめよ（補足の `(5 : Fin 3)` と
   同じ仕組みである）。
7. （補足の確認）`#eval (first 5).val + (2 : Fin 3).val` の値を予想してから確かめよ。
-/
...
1. `n + 1 - 1 = n` は（`n - 1 + 1 = n` と違って）すべての `n` で正しい。
   ...
   実際の値を渡す必要がある——`Intro1a.lean` 4節の括弧の読み分けの復習である。）
