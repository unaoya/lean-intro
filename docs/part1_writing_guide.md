...
- ▼`(2:Real)` エラー回収（Ch3）・▼trivialPartition の sorry 消える（Ch5）・▲菱形の排他分割（0/1/2 以上＝AtLeastTwo 手作り）・**defeq 観察**・▲**cast は構造を保つ射**（`IsNatHom`＝0/1/+/×/≤ を保つ準同型・`cast_isHom`。Ch7 の `IsLinearMap` と並ぶ「射」の述語。Σ可換は RS 計算で必要時に追加）
...
- 演習候補: 分点が i/n の確認（show）・cast 一括ルートを試して壊す・`cast_mul` を自分で
