...
- **(a) 機能初出の単調性**: §3 の「新機能」に挙がるのは**前章までに登場していない**機能だけ。既出機能を「新規」として説明していないか。簡単→難しいの順が崩れていないか（例: tactic mode は Ch6 まで出さない）
