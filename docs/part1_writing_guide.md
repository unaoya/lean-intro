...
- ▼`(2:Real)` エラー回収（Ch3）・▲菱形の排他分割（0/1/2 以上＝AtLeastTwo 手作り）・🪟defeq 観察・▲**diamond 事件**（同型に 2 インスタンスで機構が黙って選ぶ）・▲**cast を「順序写像」「順序半環の射」として抽象化し補題を一般論から出す（2026-06-16）**: **`Monotone φ`**（順序写像 `∀ a b, a≤b→φ a≤φ b`）を抽象定義し **`cast_le : Monotone (Nat-cast)`**（基盤・`b=a+k` の k 帰納・各ステップ `cast m≤cast m+1`・`cast_nonneg` 不使用が鍵）。**`IsOrderedSemiringHom`**（旧 IsNatHom 改名・map_zero/one/add/mul＋monotone＝順序半環の射・`cast_isHom`・`cast_summation`＝Σ と可換）。**順序系は射の一般論**（cast 非依存）: `nonneg`=monotone+map_zero・**`succ_step`(φ n<φ(n+1))=map_add+map_one+0<1**＝strict の鍵・`strictMono`(lt)=monotone+succ_step・`injective`=strictMono+三分法。cast へは `cast_nonneg/lt/inj=cast_isHom.{nonneg,strictMono,injective}`・cast 固有は `cast_le`＋`cast_add`/`mul` だけ（Ch10 IsLinearMap／RS 単調性と同じ「中核概念から帰結」）・▲除法補題（純 ring は my_ring＝Ch8 の `/` 対応 reify で・inv 相殺は Field.mul_inv）
...
- **cast は射（2026-06-15）**: ① `sum_id` は Nat の恒等式 `sum_id_nat`＝**C10_Induction**② **cast を「順序半環の射」として明示**＝C11_Numbers（`Monotone`＝順序写像・**`IsOrderedSemiringHom`**（旧 IsNatHom 改名）＋`cast_isHom`・`cast_summation`＝Σ と可換。**Ch10** IsLinearMap と並ぶ「射」の述語。**順序系 nonneg/lt/inj は射の一般論から**・cast 固有は `cast_le`＋`cast_add`/`mul` だけ・2026-06-16）③ **代表点を左端・右端に取る一般化**＝C05。`equalPartitionRepr` は `leftRepr` を適用。監査例は `cast_mul`（C11・C17 とも・[Real, instLOF] 古典ゼロ）。**等分点の formula の扱いは別途検討（ユーザー留保）**
