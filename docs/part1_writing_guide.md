...
> ⚠ **最新確定は構成 v7（2026-06-16・Ch2「型を作る（型の構成と CH 対応）」新設）**＝第 I 部 Ch1–13／第 II 部 Ch14–18。
> v7 の章立て・設計判断は `docs/textbook_plan.md` の「★ テキスト構成 v7」節を参照。
> v6 からの変更: **Ch1（命題=型）の後に新 Ch2（型の構成＝積×↔∧・和⊕↔∨・関数→↔→・帰納型 Three・商整数・
> 導入除去の CH 対応）を新設**し全体を +1 改番。Ch3 構造に直積/Int/関数型モノイドを追加。
> **以下 §3 の章別ブリーフは旧構成（v4–v6）ベースで、v7 へのブリーフ改訂は各章執筆時に同期実施する**
> （v6→v7 対応: 新 v7 Ch2＝型の構成／v6 Ch2 構造→v7 Ch3／v6 Ch3 実数→v7 Ch4／… 第I部以降すべて +1／
> v6 第II部 Ch13–17→v7 Ch14–18）。
> コード再構成は実装済（2026-06-16・ビルド clean 44 jobs）。本ガイドは第 I 部（型の構成〜リーマン和の性質）を扱う。
