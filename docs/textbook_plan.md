...
- **Ch0–Ch8**: v1 素材庫の同番章のとおり（連鎖構造・ビート・双子章・クリフハンガー・3 段の梯子・肥大時 2 分割予約、すべて維持）。試作からの更新点:
  - **Ch4 の Summation の型シグネチャ（2026-06-12 ユーザー決定）**: `{α : Type} [Add α] [OfNat α 0]` の汎用形・**n は explicit** を維持する。論点ごと教材化する: ① 数学的な最小契約は「二項演算とゼロの値」だが、mathlib 非依存の世界に `Zero` クラスは無く、core でゼロを名指しする標準の語彙が `OfNat α 0`（リテラル機構）である——「契約の正直さ」と「Lean の慣用」のトレードオフを Ch4 のコラム/演習に（代替案: Real 特化／自作 Zero クラス＝ゼロへの道が 2 本になるミニ・ダイヤモンドの種、も演習素材）。② n を implicit にできるか・すると何が起きるか（依存型の見せ場が画面から消える・rw のパターン制御）も演習に。implicit 引数の扱いは 3 段配置: **Ch2 で読む**（署名の {} と []）→ **Ch5 で初めて自分の定義に書く**（Partition.length の {n} {a b}）→ **Ch6 で機構の種明かし**（apply=メタ変数、`_` も暗黙引数も同じ穴）。
