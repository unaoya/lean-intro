...
### ★ テキスト構成 v7（最新確定・2026-06-16）— Ch2「型を作る（型の構成と CH 対応）」を新設

v6 から、ユーザー方針で **Ch1（命題=型）の後に新章 Ch2「型を作る」を新設**し全体を +1 改番。型の構成（積・和・関数・帰納型・商）を一望し、各構成の「導入（項を作る＝その型への関数）」「除去（項を使う＝その型からの関数）」を命題の論理結合子（∧・∨・→）と対比＝Curry-Howard 対応の骨格を早く見せる。

| 章 | 内容 | ファイル | 由来（v6）|
|---|---|---|---|
| Ch1 | 命題=型・証明=項 | C01 | 不変 |
| **Ch2（新）** | **型を作る — 型の構成と CH 対応**（積×↔∧・和⊕↔∨・関数→↔→・帰納型 有限集合 Three・商 (Nat×Nat)/~・導入除去）| C02_Types | 新規 |
| Ch3 | 数学的構造と class（直積/Int/**関数型モノイド** funNatAdd を追加）| C03_Structures | v6 C02 |
| Ch4 | 実数を公理で読む | C04_Axioms | v6 C03 |
| Ch5 | リーマン和の定義 | C05_Summation | v6 C04 |
| Ch6 | 数学的構造とその射 | C06_Structures | v6 C05 |
| Ch7–10 | 道具（タクティク/書換/自動化/順序）| C07–C10 | v6 C06–C09 |
| Ch11 | 帰納法+加群線形性+性質 | C11_Induction | v6 C10 |
| Ch12 | 数の体系（cast+等分割）| C12_Numbers | v6 C11 |
| Ch13 | y=x（到達点②）| C13_Example | v6 C12 |
| Ch14–18 | 第II部 | C14–C18 | v6 C13–C17（+1）|

**v7 の設計判断**:
1. **型を作る（Ch2）と構造を載せる（Ch3）を分離**: Ch2 は型の構成と CH 対応（導入除去）の一望、Ch3 は構造（CommMonoidStr・class）。帰納型の再帰・recursor 本格は Ch5（Nat.rec→Summation）、商の演算 well-defined は発展。
2. **CH 対応を早く体系的に**: 型の ×⊕→ と Prop の ∧∨→ を一枚で対比。「導入＝そこへの関数・除去＝そこからの関数」が型と命題で同じ機構→後で intro/cases/⟨⟩ が両方で同じタクティク（柱 B「仕組みの解説」の中核）。
3. **構造は型を超える**: 直積 Nat×Nat（prodNatAdd）・関数型 Three→Nat（funNatAdd・Ch6 funModule 伏線）に CommMonoidStr を載せ interchange が効く。core が直積/関数型に Add を与えなくても自分で載せる。Int も core にある。
4. **全体 +1 改番**: 第I部 Ch1–13・第II部 Ch14–18。

**実装済（2026-06-16）**: コード改番 C02–C17→C03–C18・新 C02_Types・関数型モノイド・ヘッダ/本文 ChN +1・ビルド clean（44 jobs）・md 全面改番（新 ch02_types・SUMMARY）。

---

### テキスト構成 v6（2026-06-16・v7 へ更新／素材庫）— Ch5「数学的構造とその射」を新設
...
**設計原理: 「定義 → 道具を訓練 → 帰納法で性質を証明」。** Summation の定義は `Nat.rec`（構造的再帰）、RiemannSum の定義は `Summation` ＋ `Partition`(structure)——どれも**帰納型の recursor／structure で値を作る＝定義**であり、`induction` タクティク（命題を証明する）とは用途が別。定義は道具なしで書けるので先に出し、性質の証明（道具・induction）は道具を揃えてから。
...
| Ch0 | 環境構築 | — | 不変 |
| Ch1 | 命題＝型・証明＝項 | C01 | 不変 |
| Ch2 | 構造と class | C02 | 不変 |
| Ch3 | 実数を公理で読む | C03 | 不変 |
| **Ch4** | **帰納型と構造的再帰・分割とリーマン和の定義**（Range・Summation・Partition・RS・Module の定義／recursor の一般論／trivialPartition の sorry）| C04 | 旧 C04 定義部 ＋ 旧 C05 定義部（**統合**）|
| **Ch5** | タクティク入門（trivialPartition の sorry を埋める）| C05 | 旧 C06 |
| **Ch6** | 書き換えと 2 つの等しさ | C06 | 旧 C07 |
| **Ch7** | 自動化と自作タクティク | C07 | 旧 C08 |
| **Ch8** | 順序と calc | C08 | 旧 C09 |
| **Ch9** | **帰納法**（Nat.rec＋induction）＋加群＋Σコーパス＋**リーマン和の性質**（線形性・nonneg・両側＝到達点③）| C09 | 旧 C10 ＋ 旧 C04/C05 証明部 ＋ 旧 C13（**統合**）|
| **Ch10** | 数の体系（cast）| C10 | 旧 C11 |
| **Ch11** | 具体例 y=x（**到達点②**）| C11 | 旧 C12 |
| **第 II 部** | アルキメデス〜FTC | C12–C16 | 旧 C14–C18（番号 −2）|

**v5 の設計判断（v4 からの変更と理由）**:
1. **定義先出し**: Summation/RiemannSum の定義（recursor・structure）を Ch4 に集約。性質の証明（induction・道具）を Ch9 に分離。Ch4 で「recursor は定義にも証明にも使える」と予告し、Ch9 で「同じ recursor を induction として証明に走らせる」と回収——`Nat.rec` と `induction` タクティクの両方が一本につながる。
2. **道具を対象の前に**: タクティク→書き換え→自動化→順序（Ch5–8）を Σ の性質証明（Ch9）の前に揃える。道具は Real のスカラー代数だけを題材にするので Summation 不要。
3. **Σコーパス全部を Ch9 に**: 順序（Ch8）が帰納法（Ch9）の前なので、線形性（additive/mul_left/isLinear）も**順序系（nonneg/le）も**一緒に証明できる。
4. **加群（Module）**: 旧 VectorSpace を Module に。**関数空間 α→V の加群構造を行き先 V から点ごと誘導する一般 instance**（`[Module V] : Module (α → V)`）で、Range n→Real・Real→Real が自動で加群に。Module class＋誘導 instance は Ch4（定義）、`summation_isLinear` 等は Ch9（証明）。
5. **到達点 ③→②**: RS の性質（③）は cast 不要なので Ch9。y=x 具体計算（②）は cast 必要なので Ch10 の後＝Ch11。**具体計算は RS の性質を使わない**（Σ の線形性 `summation_mul_left` と cast で直接計算）ので、依存的に ③→② が素直（一般→具体）。
6. **trivialPartition の sorry**: Ch4（定義）で提示、Ch5（タクティク入門）の"初仕事"で埋める（動機が直前）。

**v5 で解消した v4 の 3 課題**: ①タクティク入門（Ch5）の初仕事が trivialPartition（Ch4）の sorry でそのまま使える ②順序系 Σ コーパスが順序（Ch8）の後の帰納法（Ch9）で証明できる ③リーマン和の定義（到達点①）が Ch4 で早く出て動機が前に来る。

**実装方針**: 本 docs 確定 → SUMMARY/guide §3 を v5 で改訂 → 段階的にコード改番（ファイル名・import 連鎖・内容の分離=定義↔証明・タクティク題材差し替え）。
...
- **Ch0–Ch8**: v1 素材庫の同番章のとおり（連鎖構造・ビート・双子章・クリフハンガー・3 段の梯子・肥大時 2 分割予約、すべて維持）。試作からの更新点:
  - **Ch4 の Summation の契約と Zero/One の配線（2026-06-12 ユーザー決定・実装済み）**: Summation は最小契約 `{α : Type} [Add α] [Zero α]`・**n は explicit**。**発見: core（v4.30）には `Zero`/`One` クラスと一方向 bridge（`Zero.toOfNat0`/`One.toOfNat1`）が mathlib から昇格して入っている**——自作不要で、公理の zero/one を登録するだけ（C02: `instance : Zero Real := ⟨AddCommGroup.zero⟩`、C06: `instance : One Real := ⟨MulCommMonoid.one⟩`。直書きの OfNat インスタンスは廃止）。教材化: ① 意味論のクラス（Zero/One）とリテラル整形の窓口（OfNat）の分離、bridge は一方向・値は defeq という**良い菱形の規律**（C03 の悪い菱形トイデモと対）。② n を implicit にできるか（依存型の見せ場・rw 制御）は Ch4 演習。implicit 引数の 3 段配置: **Ch2 で読む** → **Ch5 で初めて書く**（Partition.length）→ **Ch6 で機構の種明かし**。
  - **Ch7 の脇道「Σ は線形形式」（2026-06-12 ユーザー指定・実装済み）**: additive_summation と summation_mul_left を「有限数列のベクトル空間上の線形形式」として言い直す脇道を C07 末尾に実装（ANCHOR: vector_space / summation_linear）。`class VectorSpace (V) extends Add V, Neg V, Zero V, SMul Real V`（公理 8 本・`•` は core の SMul）を自作し、関数空間 Range n → Real への各点インスタンス（証明は全部 funext＋Real 補題 1 行）と Real 自身のインスタンスを与え、`summation_isLinear` の証明が corpus の 2 本をペアにするだけであることを見せる——「概念の定義が既証明を束ねる」の実例＋class 設計の応用＋funext の活躍どころ。発展演習: RiemannSum の f-線形性（Ch9 の言い直し）。mathlib 対応（Module/LinearMap）は付録 C。
  - **Ch8 の defeq 観察コード（2026-06-12 ユーザー指定・実装済み）**: 「数の 2 つの建て方」の分かれ目を機械で見せるデモを C08 に実装（ANCHOR: cast_defeq）——`ofNat 0 = 0 := rfl`（defeq）・`ofNat 1 = 0 + 1 := rfl`（中身）・`#check_failure (rfl : ofNat 1 = 1)`（**defeq でないこと自体がビルドで検証される**）・cast_one := zero_add' 1（命題的等式）。
  - **Ch8 の「数の 2 つの建て方」（2026-06-12 議論の記録）**: (i) **代数一次**（採用・mathlib 式）——0/1 は代数のフィールドが一次、cast（Real.ofNat）はその上に建てる。リテラルは **0/1/2 以上で担当を排他分割**（C02/C06/C08 の 3 インスタンス＝mathlib `Nat.AtLeastTwo` の手作り版）。重なる点は `cast 0 = 0` が**構成により rfl**、`cast 1 = 1` は zero_add' で命題的（succ_ofNat が rfl なのはこの建て方のおかげ）。(ii) **cast 一括ルート**——全リテラルを `OfNat Real n := ⟨Real.ofNat n⟩` で配線すると `(1:Real) ≡ 0 + 1` となり代数の one と **defeq でない**（公理に計算はない）→ one 系の補題全部に命題的な橋が要り rfl 証明が死ぬ。**ダイヤモンドの扱いの一般原則**: ①重なる経路は defeq に揃える ②揃わないなら担当を排他分割して重なり自体を消す——mathlib が AtLeastTwo を持つ理由は (ii) の失敗として説明できる。演習: cast 一括を実際に試して何が壊れるか観察。菱形の縦糸: C03 悪い菱形 → C02/C06 良い配線（bridge）→ C08 排他分割 → 付録 C で mathlib 実物（Zero.toOfNat0・AtLeastTwo・Finset.sum が AddCommMonoid を要る理由=商と well-definedness）。
